...
// Calls our own Vercel serverless functions (/api/scores, /api/odds)
// which proxy to Sportradar + The Odds API server-side — no CORS issues

import { useState, useEffect, useCallback, useRef } from 'react'

const SCORES_INTERVAL = 30_000   // 30s
const ODDS_INTERVAL   = 60_000   // 60s
...
const transformSRMatch = (summary) => {
  ...
  return {
    ...
    gameState,
    stats:       matchStats,
    result:      mappedStatus === 'final' ? { winner: homeSets > awaySets ? 'home' : 'away' } : null,
  ...
  const fetchMatches = useCallback(async () => {
    ...
    try {
      ...
      const [liveRes, schedRes] = await Promise.all([
        fetch('/api/scores?endpoint=schedules/live/summaries', { signal }),
        fetch(`/api/scores?endpoint=schedules/${today}/summaries`, { signal }),
      ])
      ...
      if (liveJson.error || schedJson.error) {
        throw new Error(liveJson.error || schedJson.error)
      }
  ...
  const fetchOdds = useCallback(async () => {
    try {
      ...
      const results = await Promise.allSettled(
        sports.map(sport => fetch(`/api/odds?sport=${sport}`).then(r => r.ok ? r.json() : []))
      )
