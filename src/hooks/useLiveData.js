...
const SCORES_INTERVAL = 30_000
const ODDS_INTERVAL   = 60_000
...
export function useLiveData() {
  ...
  const fetchMatches = useCallback(async () => {
    ...
    try {
      const datesToFetch = []
      for (let i = -7; i <= 3; i++) datesToFetch.push(localDateISO(i))
  ...
  useEffect(() => {
    ...
    return () => { clearInterval(t1); clearInterval(t2); abortRef.current?.abort() }
