  ...
  useEffect(() => {
    ...
    const fetchDetail = async () => {
      try {
        ...
        timeline.forEach(ev => {
          if (!ev.set_scores) return;
          const setNum = ev.set_scores.length; // 0-indexed current set

          if (!sets[setNum]) sets[setNum] = { n: setNum + 1, games: [] };

          if (ev.type === "score_change" || ev.type === "game_score") {
            const s = sets[setNum];
            const lastGame = s.games[s.games.length - 1];
            const pt = {
              h: ev.home_score !== undefined ? String(ev.home_score) : "0",
              a: ev.away_score !== undefined ? String(ev.away_score) : "0",
            };
            ...
            if (!lastGame || lastGame.winner) {
              ...
              s.games.push({
                g:      s.games.length + 1,
                server: ev.serving_team === "home" ? "home" : "away",
                score:  `${ev.home_set_score||0}-${ev.away_set_score||0}`,
                pts:    [pt],
                winner: null,
                break_: false,
              });
              ...
              // Check if game ended
              if (pt.h === "G" || pt.a === "G") {
                lastGame.winner = pt.h === "G" ? "home" : "away";
                // Break = server lost
        ...
        const detail = {
          ...
          venue:     match.venue   || "",
          duration:  sumStatus?.duration ? `${Math.floor(sumStatus.duration/60)}h ${sumStatus.duration%60}m` : "",
          home: {
            name:               match.home.name,
            country:            match.home.country,
            aces:               hStats.aces              || 0,
            dfs:                hStats.double_faults      || 0,
            firstServe:         hStats.first_serve_pct    ? `${Math.round(hStats.first_serve_pct)}%` : "—",
            firstServePtsWon:   hStats.first_serve_pct_points_won ? `${Math.round(hStats.first_serve_pct_points_won)}%` : "—",
            secondServePtsWon:  hStats.second_serve_pct_points_won ? `${Math.round(hStats.second_serve_pct_points_won)}%` : "—",
            bpWon:              `${hStats.break_points_won||0}/${hStats.break_points||0}`,
            winners:            hStats.winners            || 0,
            ufErrors:           hStats.unforced_errors    || 0,
            totalPtsWon:        hStats.points_won         || 0,
            maxStreak:          hStats.max_consecutive_points_won || 0,
          },
          away: {
            ...
            firstServe:         aStats.first_serve_pct    ? `${Math.round(aStats.first_serve_pct)}%` : "—",
            firstServePtsWon:   aStats.first_serve_pct_points_won ? `${Math.round(aStats.first_serve_pct_points_won)}%` : "—",
            secondServePtsWon:  aStats.second_serve_pct_points_won ? `${Math.round(aStats.second_serve_pct_points_won)}%` : "—",
            bpWon:              `${aStats.break_points_won||0}/${aStats.break_points||0}`,
            winners:            aStats.winners            || 0,
            ufErrors:           aStats.unforced_errors    || 0,
            totalPtsWon:        aStats.points_won         || 0,
            maxStreak:          aStats.max_consecutive_points_won || 0,
          },
          sets: matchSets,
        };
          ...
          {!detailLoad&&!detailErr&&detail&&activeTab==="points"&&(
            <div style={{padding:"10px 12px"}}>
              ...
              {set&&set.games.length>0 ? (
                ...
                <div style={{textAlign:"center",padding:24,color:"#aaa",fontFamily:"monospace",fontSize:11}}>
                  {detail.sets.length===0
                    ? "No sets played yet"
                    : "Detailed point history not available for this set"}
                </div>
              )}
            </div>
          )}
