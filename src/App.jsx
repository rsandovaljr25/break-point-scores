...
const ALL_MATCHES = []; // Seed data removed — live data only

// ── Seed picks with all 3 market types ───────────────────────────────────────
const SEED_PICKS = [
  // No seed data — leaderboard shows real users only
];

// Pre-resolve all seed picks
const RESOLVED_SEED_PICKS = [];
...
const LEVEL_ORDER={"Grand Slam":0,"Masters 1000":1,"WTA 1000":1,"ATP 500":2,"WTA 500":2,"ATP 250":3,"WTA 250":3,"Challenger":4,"UTR":5};
const getTourLevel = t => {
  if(t.includes("French Open")||t.includes("Wimbledon")||t.includes("US Open")||t.includes("Australian Open")) return "Grand Slam";
  if(t.includes("Masters")||t.includes("1000")) return "Masters 1000";
...
function MatchCard({match,pulse,isFav,onToggleFav,username,myPicks,onSubmitPick,onRemovePick,onOpenDetail,setPlayerProfile}){
  ...
  const isLive=match.status==="live"; const isFinal=["final","walkover","retired","cancelled"].includes(match.status);
      ...
      <div style={{padding:"10px 11px 7px"}}>
        ...
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginTop:7,paddingTop:6,borderTop:"1px solid #e0e4d8"}}>
          <div style={{display:"flex",alignItems:"center",gap:5}}>
            ...
            {(isFinal||["walkover","retired","cancelled"].includes(match.status))&&(
              <span style={{fontSize:9,color:"#ffffff",fontFamily:"monospace",
                background:match.status==="walkover"||match.status==="retired"?"#7b5ea7":
                           match.status==="cancelled"?"#888":"#444",
                borderRadius:3,padding:"1px 6px",letterSpacing:"0.06em",fontWeight:"bold"}}>
                {match.status==="walkover"?"WALKOVER":match.status==="retired"?"RETIRED":
                 match.status==="cancelled"?"CANCELLED":"FINAL"}
              </span>
            )}
            {(()=>{
              const sb={scheduled:{l:"SCHEDULED",bg:"#a8d828",c:"#1a1a1a"},postponed:{l:"POSTPONED",bg:"#ff8c00",c:"#fff"},delayed:{l:"DELAYED",bg:"#ff8c00",c:"#fff"},suspended:{l:"SUSPENDED",bg:"#cc3333",c:"#fff"},walkover:{l:"WALKOVER",bg:"#7b5ea7",c:"#fff"},retired:{l:"RETIRED",bg:"#7b5ea7",c:"#fff"},cancelled:{l:"CANCELLED",bg:"#888",c:"#fff"}};
              const b=sb[match.status];
              return b?<span style={{fontSize:9,color:b.c,fontFamily:"monospace",background:b.bg,borderRadius:3,padding:"1px 6px",letterSpacing:"0.06em",fontWeight:"bold"}}>{b.l}</span>:null;
            })()}
          </div>
...
function PlayerProfileModal({ playerName, playerCountry, playerTour, opponentName, onClose, isFavPlayer, onToggleFavPlayer }) {
        ...
        <div style={{background:"linear-gradient(135deg,#1a1a1a,#2a2a2a)",
          ...
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:10}}>
            ...
            <div style={{display:"flex",gap:6,flexShrink:0}}>
              <button onClick={onToggleFavPlayer} style={{background:isFavPlayer?"#2a2000":"transparent",
                border:`1px solid ${isFavPlayer?"#a8d828":"#444"}`,
                color:isFavPlayer?"#a8d828":"#888",borderRadius:6,
                width:28,height:28,cursor:"pointer",fontSize:14}}>
                {isFavPlayer?"★":"☆"}
              </button>
              <button onClick={onClose} style={{background:"transparent",border:"1px solid #444",
                color:"#aaa",borderRadius:6,width:28,height:28,cursor:"pointer",fontSize:14}}>✕</button>
            </div>
          </div>
...
const MATCH_DETAIL_DATA = {}; // Detail fetched live from API
...
function MatchDetailModal({ matchId, match, onClose }) {
  ...
  // ── Fetch timeline + summary from Sportradar on open ──────────────────────
  useEffect(() => {
    if (!matchId) return;
    setDetailLoad(true);
    setDetailErr(null);
    ...
    const fetchDetail = async () => {
      try {
        // Fetch timeline (point history) and summary (stats) in parallel
        const [timelineRes, summaryRes] = await Promise.all([
          fetch(`/api/timeline?id=${encodeURIComponent(matchId)}`),
          fetch(`/api/summary?id=${encodeURIComponent(matchId)}`),
        ]);
        ...
        console.log("Timeline response:", JSON.stringify(timelineJson)?.slice(0,500));
        console.log("Summary response:", JSON.stringify(summaryJson)?.slice(0,300));

        if (!timelineJson && !summaryJson) {
          setDetailErr("Match detail not available from data provider.");
          setDetail(null);
          return;
        }

        // ── Parse timeline into sets/games/points ────────────────────────────
        const timeline = timelineJson?.timeline || timelineJson?.sport_event_timeline?.timeline || [];
        console.log("Timeline events count:", timeline.length);
        if (timeline.length > 0) {
          console.log("First event types:", timeline.slice(0,5).map(e=>e.type).join(", "));
          console.log("First point event:", JSON.stringify(timeline.find(e=>e.type==="point")||timeline[0]));
        }

        // Tennis score display map: Sportradar sends numeric 0-4 or string labels
        const TENNIS_PTS = { 0:"0", 1:"15", 2:"30", 3:"40", 4:"G", 50:"A",
                             "0":"0","15":"15","30":"30","40":"40","A":"A","AD":"A","G":"G" };
        const tennisPt = v => {
          if (v === undefined || v === null) return "0";
          const key = typeof v === "string" ? v.toUpperCase() : v;
          return TENNIS_PTS[key] ?? String(v);
        };

        const parsedSets = {};

        timeline.forEach(ev => {
          // Sportradar uses "point" type for each point played
          if (ev.type !== "point") return;

          // Determine which set this point belongs to
          const setScores = ev.set_scores || [];
          const setIdx    = Math.max(0, setScores.length - 1);

          if (!parsedSets[setIdx]) parsedSets[setIdx] = { n: setIdx + 1, games: [] };
          const s = parsedSets[setIdx];

          // Current set game score
          const curSet     = setScores[setIdx] || {};
          const hSetScore  = curSet.home_score ?? 0;
          const aSetScore  = curSet.away_score ?? 0;

          // Point scores within the game - properly formatted
          const hPt = tennisPt(ev.home_score);
          const aPt = tennisPt(ev.away_score);

          const server = ev.serve_qualifier === "home" || ev.serve === "home" ||
                         ev.serving === "home" || ev.server === match.home.id ? "home" : "away";

          const lastGame = s.games[s.games.length - 1];
          const gameKey  = `${hSetScore}-${aSetScore}`;

          if (!lastGame || lastGame.gameKey !== gameKey) {
            // New game
            s.games.push({
              g:       s.games.length + 1,
              server,
            ...
            lastGame.pts.push({ h: hPt, a: aPt });
            // Detect game won: when score is "G" on either side
            if (hPt === "G" || aPt === "G") {
              lastGame.winner = hPt === "G" ? "home" : "away";
              if (lastGame.winner !== lastGame.server) lastGame.break_ = true;
            }
          }
        });

        // Build set scores merging match data with parsed timeline games
        const matchSets = (match.sets || []).map((s, i) => ({
          n:     i + 1,
          h:     s.h,
          a:     s.a,
          games: parsedSets[i]?.games || [],
        }));

        // ── Parse summary stats ───────────────────────────────────────────────
        const sumStatus = summaryJson?.sport_event_status;
        const sumStats  = summaryJson?.statistics;
        const hStats = sumStats?.totals?.competitors?.find(c => c.qualifier === "home")?.statistics || {};
        const aStats = sumStats?.totals?.competitors?.find(c => c.qualifier === "away")?.statistics || {};

        // Calculate percentages from raw counts (Sportradar gives counts not pcts)
        const servePct = (won, total) => total > 0 ? `${Math.round((won/total)*100)}%` : "—";
        const hTotalSvc  = (hStats.first_serve_successful||0) + (hStats.second_serve_successful||0);
        const aTotalSvc  = (aStats.first_serve_successful||0) + (aStats.second_serve_successful||0);
        const h1stPct    = servePct(hStats.first_serve_successful||0, hTotalSvc);
        const a1stPct    = servePct(aStats.first_serve_successful||0, aTotalSvc);
        const h1stWinPct = servePct(hStats.first_serve_points_won||0, hStats.first_serve_successful||0);
        const a1stWinPct = servePct(aStats.first_serve_points_won||0, aStats.first_serve_successful||0);
        const h2ndWinPct = servePct(hStats.second_serve_points_won||0, hStats.second_serve_successful||0);
        const a2ndWinPct = servePct(aStats.second_serve_points_won||0, aStats.second_serve_successful||0);

        const detail = {
          status:    match.status,
          surface:   match.surface || "Hard",
          venue:     match.venue   || "",
          duration:  sumStatus?.clock ? `${Math.floor(sumStatus.clock/60)}h ${sumStatus.clock%60}m` : "",
          home: {
            ...
            firstServe:         h1stPct,
            firstServePtsWon:   h1stWinPct,
            secondServePtsWon:  h2ndWinPct,
            bpWon:              `${hStats.breakpoints_won||0}/${hStats.total_breakpoints||0}`,
            winners:            hStats.winners                || 0,
            ufErrors:           hStats.unforced_errors        || 0,
          ...
          away: {
            ...
            firstServe:         a1stPct,
            firstServePtsWon:   a1stWinPct,
            secondServePtsWon:  a2ndWinPct,
            bpWon:              `${aStats.breakpoints_won||0}/${aStats.total_breakpoints||0}`,
            winners:            aStats.winners                || 0,
            ufErrors:           aStats.unforced_errors        || 0,
        ...
        setDetail(detail);
        if (matchSets.length > 0) setActiveSet(matchSets.length - 1);

      } catch(e) {
        console.error("Detail fetch error:", e);
        setDetailErr("Could not load match detail. Please try again.");
      } finally {
        setDetailLoad(false);
      }
    };
    ...
    const interval = isLive ? setInterval(fetchDetail, 30000) : null;
    ...
    <div style={{position:"fixed",inset:0,background:"#00000099",zIndex:200,
      ...
      <div style={{background:"#ffffff",borderRadius:"16px 16px 0 0",width:"100%",maxWidth:560,
        ...
        <div style={{background:"linear-gradient(135deg,#1a1a1a,#2a2a2a)",padding:"14px 16px 12px",flexShrink:0}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:10}}>
            <div>
              <div style={{fontSize:9,color:"#a8d828",fontFamily:"monospace",letterSpacing:"0.15em",marginBottom:2}}>
          ...
          {/* Players */}
          {[{player:match.home,name:hn,won:homeWon,sw:match.setsWon?.home??0},
            {player:match.away,name:an,won:awayWon,sw:match.setsWon?.away??0}].map(({player,name,won,sw},pi)=>(
            <div key={pi} style={{display:"flex",alignItems:"center",gap:8,marginBottom:pi===0?6:0}}>
              ...
              <span style={{fontSize:11}}>{flagEmoji(player.country)}</span>
              <div style={{flex:1}}>
                <span style={{fontSize:15,fontWeight:"bold",color:won?"#a8d828":"#e0eaf4"}}>{name.last}</span>
                <span style={{fontSize:10,color:"#888",marginLeft:6}}>{name.first}</span>
              </div>
              {/* Set scores */}
              ...
              <div style={{width:22,height:22,display:"flex",alignItems:"center",justifyContent:"center",
                ...
                fontSize:14,fontWeight:"bold",fontFamily:"monospace",color:won?"#a8d828":"#888"}}>
                {sw}
              </div>
            </div>
          ))}
        ...
        <div style={{overflowY:"auto",flex:1}}>
          ...
          {detailLoad&&(
            <div style={{textAlign:"center",padding:40}}>
              <div style={{fontSize:12,color:"#a8d828",fontFamily:"monospace",letterSpacing:"0.1em"}}>
                ⚡ LOADING MATCH DATA…
              </div>
            </div>
          )}
          ...
          {!detailLoad&&detailErr&&(
            <div style={{textAlign:"center",padding:32}}>
              ...
              <div style={{fontSize:10,color:"#aaa",fontFamily:"monospace"}}>
                Point history is available for matches played within the trial API window.
              </div>
            </div>
          )}
          ...
          {!detailLoad&&!detailErr&&detail&&activeTab==="points"&&(
            <div style={{padding:"10px 12px"}}>
              ...
              <div style={{display:"flex",gap:8,marginBottom:8,fontSize:9,fontFamily:"monospace"}}>
                ...
                <span style={{color:"#aaa",marginLeft:"auto"}}>● = server · ⚡ = break</span>
              </div>

              {set&&set.games.length>0 ? (
                <div style={{display:"flex",flexDirection:"column",gap:3}}>
                  {set.games.map((game,gi)=>(
                    <div key={gi} style={{
                      background:"#fafaf7",
                      border:`2px solid ${game.break_?"#d94f2a":game.live?"#a8d828":"#e8e8e4"}`,
                      borderRadius:6,padding:"5px 8px",
                      boxShadow:game.break_?"0 1px 6px #d94f2a22":"none"}}>
                      <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:3,flexWrap:"wrap"}}>
                        <span style={{fontSize:9,color:"#5a7a5a",fontFamily:"monospace",minWidth:24}}>G{game.g}</span>
                        <span style={{fontSize:8,color:"#ffaa22",fontFamily:"monospace"}}>
                          {game.server==="home"?`${hn.last} ●`:`● ${an.last}`} srv
                        </span>
                        <span style={{fontSize:8,color:"#888",fontFamily:"monospace"}}>{game.score}</span>
                        {game.break_&&<span style={{fontSize:8,color:"#d94f2a",fontFamily:"monospace"}}>⚡BRK</span>}
                        {game.winner&&<span style={{fontSize:8,fontFamily:"monospace",marginLeft:"auto",
                          color:game.winner==="home"?"#a8d828":"#44cc88"}}>
                          {game.winner==="home"?hn.last:an.last} wins
                        </span>}
                      </div>
                      {/* Point sequence */}
                      <div style={{fontSize:10,fontFamily:"monospace",lineHeight:1.5,wordBreak:"break-word"}}>
                        {game.pts.map((pt,pi2)=>{
                          ...
                          return(
                            <span key={pi2}>
                              <span style={{color:pt.h==="G"?"#a8d828":hWon?"#6a9a30":"#5a7a5a",fontWeight:isFinal?"bold":"normal"}}>{pt.h}</span>
                              <span style={{color:"#ccc"}}>-</span>
                              <span style={{color:pt.a==="G"?"#44cc88":aWon?"#2a7a4a":"#4a6a5a",fontWeight:isFinal?"bold":"normal"}}>{pt.a}</span>
                              {pi2<game.pts.length-1&&<span style={{color:"#ddd"}}> · </span>}
                            </span>
                          );
                        })}
                      </div>
                ...
                <div style={{textAlign:"center",padding:24}}>
                  <div style={{fontSize:11,color:"#aaa",fontFamily:"monospace",marginBottom:6}}>
                    {detail.sets.length===0 ? "No sets played yet" : "Point-by-point data not available"}
                  </div>
                  <div style={{fontSize:9,color:"#ccc",fontFamily:"monospace",lineHeight:1.6}}>
                    Play-by-play coverage varies by tournament.<br/>
                    Grand Slams and Masters events have full point history.<br/>
                    Match stats are available in the STATS tab.
                  </div>
                </div>
              )}
            </div>
...
export default function App(){
  ...
  const { matches: liveMatches, oddsData: liveOdds, loading: dataLoading,
          error: dataError, lastUpdated, refresh } = useLiveData();

  // Use live matches if available, fall back to seed ALL_MATCHES
  const rawMatches = liveMatches; // live data only — no static fallback
  // Debug log
  if (typeof window !== 'undefined') {
    console.log('MATCHES loaded:', liveMatches.length, 'dataLoading:', dataLoading, 'error:', dataError);
  }

  // ── Wire odds into match.markets ─────────────────────────────────────────
  const MATCHES = rawMatches.map(match => {
    if (!liveOdds?.length) return match;

    // Normalize name: "Lastname, Firstname" → "firstname lastname" for fuzzy match
    const normName = name => {
      if (!name) return '';
      const parts = name.split(', ');
      return (parts.length === 2 ? `${parts[1]} ${parts[0]}` : name).toLowerCase();
    };

    const hName = normName(match.home.name);
    const aName = normName(match.away.name);
    const hLast = hName.split(' ').pop();
    const aLast = aName.split(' ').pop();

    // Find matching odds event
    const oddsEvent = liveOdds.find(ev => {
      const ht = (ev.home_team || '').toLowerCase();
      const at = (ev.away_team || '').toLowerCase();
      return (ht.includes(hLast) || at.includes(aLast) ||
              ht.includes(aLast) || at.includes(hLast));
    });

    if (!oddsEvent) return match;

    // Find best bookmaker
    const preferred = ['draftkings','fanduel','betmgm','pointsbetus','williamhill'];
    const bm = oddsEvent.bookmakers?.find(b => preferred.includes(b.key))
    ...
    const htLast = (oddsEvent.home_team || '').toLowerCase().split(' ').pop();
    const homeOut = h2h.outcomes?.find(o => o.name?.toLowerCase().includes(htLast));
    const awayOut = h2h.outcomes?.find(o => o !== homeOut);

    if (!homeOut || !awayOut) return match;

    const toML = dec => dec >= 2
    ...
    return {
      ...
      markets: {
        ...match.markets,
        ml: {
          home: { dec: homeOut.price, ml: parseInt(toML(homeOut.price)) },
          away: { dec: awayOut.price, ml: parseInt(toML(awayOut.price)) },
        }
      }
    };
  });
  ...
  const [favPlayers,   setFavPlayers]   = useState(new Set()); // favorited player names
  ...
  const shown=(()=>{
    ...
    if(mainTab==="FAVORITES") {
      list = MATCHES.filter(m=>favorites.has(m.id)||favPlayers.some(p=>m.home.name===p||m.away.name===p));
    } else if(mainTab==="PICKS") {
      list = MATCHES.filter(m=>Object.values(myPicks).some(p=>p.matchId===m.id)||m.status!=="final");
    } else if(showLiveOnly) {
      list = MATCHES.filter(m=>m.status==="live");
    } else if(selectedDate) {
      // Date selected — show ALL matches on that date regardless of status
      list = MATCHES.filter(m=>(m.startDate||m.startTime?.slice(0,10))===selectedDate);
    } else {
      // Default — show today's matches (all statuses) + upcoming scheduled
      list = MATCHES.filter(m=>{
        const matchDate = m.startDate || m.startTime?.slice(0,10);
        // Always show live matches regardless of date
        if(m.status==="live") return true;
        // Show today's matches (any status including final)
        if(matchDate===localToday) return true;
        // Show upcoming scheduled/delayed for next 7 days
        if(matchDate>localToday && ["scheduled","delayed","postponed"].includes(m.status)) return true;
        return false;
      });
    }
    ...
    // ── Sort: Live → Scheduled → Final, then by level, then by start time ──
    return list.sort((a,b)=>{
      // Status priority
      const sOrder = {live:0,delayed:1,suspended:1,scheduled:2,postponed:2,final:3,walkover:3,retired:3,cancelled:3};
      const sd = (sOrder[a.status]??9) - (sOrder[b.status]??9);
      if(sd !== 0) return sd;

      // Within same status group: sort finals by start time ascending
      // (earliest finished match at top, most recent at bottom)
      if(a.status==="final"||a.status==="walkover"||a.status==="retired") {
        return (a.startTime||"").localeCompare(b.startTime||"");
      }

      // For live/scheduled: sort by level then start time
      const la = LEVEL_ORDER[getTourLevel(a.tournament)]??9;
      const lb = LEVEL_ORDER[getTourLevel(b.tournament)]??9;
  ...
  return(
    <div style={{minHeight:"100vh",background:"#f5f5f2",color:"#2a2a2a",fontFamily:"Georgia,serif",display:"flex",flexDirection:"column"}}>
      ...
      <div style={{background:"linear-gradient(180deg,#1c1c1c,#111111)",borderBottom:"2px solid #a8d828",padding:"13px 13px 0",position:"sticky",top:0,zIndex:20}}>
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:9}}>
          <div style={{display:"flex",alignItems:"center",gap:8}}>
            <img src="/logo.png" onError={e=>{e.target.src="data:image/svg+xml,%3Csvg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 100 100'%3E%3Ccircle cx='50' cy='50' r='48' fill='%231a1a1a' stroke='%23a8d828' stroke-width='3'/%3E%3Ctext x='50' y='58' text-anchor='middle' font-family='Arial Black' font-size='12' fill='%23a8d828'%3EBP%3C/text%3E%3C/svg%3E";}} alt="Break Point Scores" style={{width:44,height:44,borderRadius:"50%",objectFit:"cover",border:"2px solid #a8d828",flexShrink:0}}/>
            <div>
              ...
              <div style={{fontSize:8,color:"#6a8a3a",letterSpacing:"0.12em",fontFamily:"monospace"}}>
          ...
          <div style={{paddingTop:6,paddingBottom:4}}>
              ...
              {mainTab==="SCORES"&&(
                <div style={{marginLeft:"auto",display:"flex",gap:5,alignItems:"center"}}>
                  <button onClick={()=>{setShowLiveOnly(l=>!l);setSelectedDate(null);}} style={{display:"flex",alignItems:"center",gap:4,background:showLiveOnly?"#1a1a1a":"transparent",border:`2px solid ${showLiveOnly?"#a8d828":"#d0d0cc"}`,color:showLiveOnly?"#a8d828":"#888",borderRadius:6,padding:"3px 10px",fontSize:9,fontFamily:"monospace",cursor:"pointer",fontWeight:"bold"}}>
                    <span style={{width:6,height:6,borderRadius:"50%",background:showLiveOnly?"#a8d828":"#ccc",display:"inline-block",flexShrink:0}}/>
                    LIVE
                  </button>
                  <button onClick={()=>setShowDatePicker(d=>!d)} style={{display:"flex",alignItems:"center",gap:4,background:selectedDate?"#1a2a05":"transparent",border:`1px solid ${selectedDate?"#a8d828":"#d0d0cc"}`,color:selectedDate?"#a8d828":"#888",borderRadius:6,padding:"3px 10px",fontSize:9,fontFamily:"monospace",cursor:"pointer"}}>
                    ...
                    {selectedDate&&<span onClick={(e)=>{e.stopPropagation();setSelectedDate(null);setShowDatePicker(false);}} style={{marginLeft:4,color:"#cc3333",fontWeight:"bold",fontSize:10}}>✕</span>}
                  </button>
                </div>
              )}
            </div>
            {showDatePicker&&mainTab==="SCORES"&&(
              <div style={{marginTop:6,background:"#ffffff",border:"2px solid #a8d828",borderRadius:10,padding:10,boxShadow:"0 4px 16px #00000018"}}>
                <div style={{fontSize:9,color:"#888",fontFamily:"monospace",marginBottom:6,letterSpacing:"0.08em"}}>SELECT DATE</div>
                <div style={{display:"flex",gap:5,flexWrap:"wrap",marginBottom:8}}>
                  {(()=>{
                    ...
                    for(let i=-7;i<=7;i++){
                      ...
                      const label=i===-1?"Yesterday":i===0?"Today":i===1?"Tomorrow":d.toLocaleDateString("en-US",{month:"short",day:"numeric"});
                      dates.push({iso,label,isPast:i<0,isToday:i===0});
                    }
                    return dates.map(({iso,label,isPast,isToday})=>(
                      <button key={iso} onClick={()=>{setSelectedDate(iso);setShowDatePicker(false);setShowLiveOnly(false);}} style={{background:selectedDate===iso?"#a8d828":isToday?"#f0f8e8":"#f5f5f2",border:`1px solid ${selectedDate===iso?"#a8d828":isToday?"#c8e070":"#e0e0dc"}`,color:selectedDate===iso?"#1a1a1a":isToday?"#2a6a2a":isPast?"#666":"#1a1a1a",borderRadius:6,padding:"4px 10px",fontSize:10,fontFamily:"monospace",cursor:"pointer",fontWeight:isToday||selectedDate===iso?"bold":"normal"}}>
                        {label}
                      </button>
                    ));
                  })()}
              ...
              return(
                <div key={tourney} style={{marginBottom:16}}>
                  ...
                  <div style={{display:"flex",alignItems:"center",gap:7,marginBottom:8,
                    ...
                    <button onClick={()=>setFavorites(f=>{const n=new Set(f);matches.forEach(m=>n.has(m.id)?n.delete(m.id):n.add(m.id));return n;})}
                      title="Favorite all matches in this tournament"
                      style={{background:"transparent",border:"1px solid #d0e0b0",borderRadius:4,
                        padding:"2px 7px",fontSize:11,cursor:"pointer",color:"#a8d828",marginLeft:"auto"}}>
                      {matches.every(m=>favorites.has(m.id))?"★":"☆"}
                    </button>
                  </div>
                  );})()}
                  {matches.map(m=>(
      ...
      {/* DATA STATUS + VISITOR BAR */}
      {dataLoading&&<div style={{background:"#eef6e0",borderTop:"1px solid #c8e070",padding:"4px 14px",display:"flex",alignItems:"center",gap:6}}><span style={{fontSize:9,color:"#5a8a2a",fontFamily:"monospace",letterSpacing:"0.06em"}}>⚡ LOADING LIVE DATA…</span></div>}
      {dataError&&!dataLoading&&<div style={{background:"#fceaec",borderTop:"1px solid #f0a0a0",padding:"4px 14px",display:"flex",alignItems:"center",justifyContent:"space-between"}}><span style={{fontSize:9,color:"#cc3333",fontFamily:"monospace"}}>⚠ Live data unavailable — showing cached data</span><button onClick={refresh} style={{fontSize:9,color:"#cc3333",fontFamily:"monospace",background:"transparent",border:"1px solid #f0a0a0",borderRadius:4,padding:"1px 8px",cursor:"pointer"}}>RETRY</button></div>}
      ...
      {playerProfile&&<PlayerProfileModal playerName={playerProfile.name} playerCountry={playerProfile.country} playerTour={playerProfile.tour} opponentName={playerProfile.opponent} onClose={()=>setPlayerProfile(null)} isFavPlayer={favPlayers.has(playerProfile.name)} onToggleFavPlayer={()=>toggleFavPlayer(playerProfile.name)}/>}
