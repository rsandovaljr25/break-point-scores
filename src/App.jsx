  ...
  const shown=(()=>{
    ...
    if(mainTab==="FAVORITES") {
      list = MATCHES.filter(m =>
        favorites.has(m.id) ||
        favPlayers.some(p => m.home.name===p || m.away.name===p));

    } else if(mainTab==="PICKS") {
      list = MATCHES.filter(m =>
        Object.values(myPicks).some(p => p.matchId===m.id) ||
        m.status !== "final");
      ...
      // Date filter — show all matches on that UTC date
      list = MATCHES.filter(m => m.startDate === selectedDate);

    } else {
      // Default — show everything from the live/summaries endpoint
      list = [...MATCHES];
    }

    // Always exclude UTR
    list = list.filter(m =>
      !m.tournament.includes("UTR") &&
      getTourInfo(m.tournament).tour !== "UTR");

    // Tour filter
    if(tourFilter !== "ALL")
      list = list.filter(m => getTourInfo(m.tournament).tour === tourFilter);

    // Sort: Live → Scheduled → Final, then by level, then by start time
    const sOrder = {live:0,delayed:1,suspended:1,scheduled:2,postponed:2,
                    final:3,walkover:3,retired:3,cancelled:3};
                ...
                <div style={{display:"flex",gap:5,flexWrap:"wrap",marginBottom:8}}>
                  {(()=>{
                    ...
                    for(let i=-7;i<=7;i++){
                      ...
                      // Use UTC to match Sportradar's date format
                      const utc=new Date(Date.UTC(d.getFullYear(),d.getMonth(),d.getDate())); const iso=utc.toISOString().slice(0,10);
