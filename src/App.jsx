...
export default function App(){
  ...
  const shown=(()=>{
    let list=mainTab==="FAVORITES"?MATCHES.filter(m=>favorites.has(m.id)||favPlayers.some(p=>m.home.name===p||m.away.name===p)):
             ...
             selectedDate
               ? MATCHES.filter(m=>(m.startDate||m.startTime?.slice(0,10))===selectedDate)
               // Default (no date): show live + scheduled + today's results
               : MATCHES.filter(m=>m.status==='live'||m.status==='scheduled'||
                   m.status==='delayed'||m.status==='postponed'||
                   (m.startDate===localToday&&m.status!=='scheduled'));
    // Always exclude UTR events
