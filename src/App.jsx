...
export default function App(){
  ...
  const shown=(()=>{
    let list=mainTab==="FAVORITES"?MATCHES.filter(m=>favorites.has(m.id)||favPlayers.some(p=>m.home.name===p||m.away.name===p)):
             ...
             mainTab==="HISTORY"?MATCHES.filter(m=>m.status==="final"):
             MATCHES.filter(m=>m.status===TAB_STATUS[scoreTab]);
  ...
  const MAIN_TABS=[["SCORES","🎾"],["PICKS",`🎯 ${pickCount}`],["LEADERBOARD","🏆"],["HISTORY","📅"],["FAVORITES",`⭐ ${favCount}`]];
