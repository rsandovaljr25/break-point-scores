  ...
  useEffect(() => {
    ...
    const fetchDetail = async () => {
      try {
        ...
        // Map numeric tennis scores to display format
        const tennisPt = v => {
          if (v === undefined || v === null) return "0";
          const n = parseInt(v);
          if (isNaN(n)) return String(v);
          const map = {0:"0", 1:"15", 2:"30", 3:"40", 4:"G", 50:"A"};
          return map[n] !== undefined ? map[n] : String(v);
        };

        timeline.forEach(ev => {
          // Sportradar point events: type "point" with set_scores array
          const isPointEv = ev.type === "point" || ev.type === "score_change" || 
                            ev.type === "game_score" || ev.type === "period_score";
          if (!isPointEv || !ev.set_scores) return;

          const setNum = (ev.set_scores.length || 1) - 1; // 0-indexed
          if (setNum < 0) return;

          if (!sets[setNum]) sets[setNum] = { n: setNum + 1, games: [] };
          const s = sets[setNum];

          // Get current game score within set
          const hSetScore = ev.home_score ?? ev.set_scores?.[setNum]?.home_score ?? 0;
          const aSetScore = ev.away_score ?? ev.set_scores?.[setNum]?.away_score ?? 0;

          // Game point scores
          const hPt = tennisPt(ev.home_game_score ?? ev.game_home_score);
          const aPt = tennisPt(ev.away_game_score ?? ev.game_away_score);
          const pt  = { h: hPt, a: aPt };

          const lastGame = s.games[s.games.length - 1];
          const server   = ev.serve_team === "home" || ev.serving_team === "home" ? "home" : "away";

          // Start new game or add to current
          if (!lastGame || lastGame.winner) {
            s.games.push({
              g:      s.games.length + 1,
              server,
              score:  `${hSetScore}-${aSetScore}`,
              pts:    [pt],
              winner: null,
              break_: false,
            });
          } else {
            lastGame.pts.push(pt);
            // Game won when score shows "G"
            if (hPt === "G" || aPt === "G") {
        ...
        // Build set scores from match data
...
function useVisitorTracker(){
  const [stats,setStats]=useState({today:1,total:1,online:1});
  useEffect(()=>{
    // Use localStorage since window.storage is artifact-only
    try {
      const today = new Date().toISOString().slice(0,10);
      const totalKey = "bps_visitors_total";
      const dayKey   = `bps_visitors_${today}`;
      let total   = parseInt(localStorage.getItem(totalKey)||"0")+1;
      let todayCount = parseInt(localStorage.getItem(dayKey)||"0")+1;
      localStorage.setItem(totalKey, String(total));
      localStorage.setItem(dayKey,   String(todayCount));
      const online = Math.min(Math.max(Math.floor(total/5)+1,1),999);
      setStats({today:todayCount, total, online});
    } catch(e) {
      setStats({today:1,total:1,online:1});
    }
  },[]);
  return stats;
}
        ...
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:9}}>
          <div style={{display:"flex",alignItems:"center",gap:8}}>
            ...
            <div>
              ...
              <div style={{fontSize:8,color:"#6a8a3a",letterSpacing:"0.12em",fontFamily:"monospace"}}>ROLAND GARROS · MAY 25, 2026</div>
            </div>
          </div>
