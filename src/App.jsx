...
export default function App(){
  ...
  const shown=(()=>{
    let list=mainTab==="FAVORITES"?ALL_MATCHES.filter(m=>favorites.has(m.id)):mainTab==="PICKS"?ALL_MATCHES.filter(m=>Object.values(myPicks).some(p=>p.matchId===m.id)||m.status!=="final"):ALL_MATCHES.filter(m=>m.status===TAB_STATUS[scoreTab]);
  ...
  const liveCount=ALL_MATCHES.filter(m=>m.status==="live").length;
